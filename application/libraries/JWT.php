...
class JWT {
    ...
    public function validateToken($token) {
        return $this->decode($token, $this->key, array($this->algorithm));
    }
    public function generateToken($data) {
        return $this->encode($data, $this->key);
    }

    private function decode($jwt, $key, $allowed_algs = array())
    {
        $_timestamp = is_null($this->timestamp) ? time() : $this->timestamp;
        if (empty($key)) {
    ...
    private function verify($msg, $signature, $key, $alg)
    {
        ...
        switch($function) {
            case 'openssl':
                ...
                if (!$success) {
                    return "OpenSSL unable to verify data: " . openssl_error_string();
                } else {
                    return $signature;
                }
            case 'hash_hmac':
